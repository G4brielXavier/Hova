...
def Parser(tokens):
    ...
    # Functions to "navigate" and manage the tokens list
    ...
    def Expect(value=None, type=None, no_error=False, msg=None):
        ...
        if not next:
            raise HovaSyntaxError(f'Expected "{value or type}" but got NULL.', tokens[pos-1].start_ln, tokens[pos-1].start_col)
        ...
        if value is not None and next.value != value:
            raise HovaSyntaxError(f'Expected token value "{value}", but got "{next.value}"' if msg is None else msg, tokens[pos-1].start_ln, tokens[pos-1].start_col)
            
        if type is not None and next.type != type:
            raise HovaSyntaxError(f'Expected token type "{type}", but got "{next.type}"' if msg is None else msg, tokens[pos-1].start_ln, tokens[pos-1].start_col)
    ...
    def Parse_Block(until:str=None, encompassName:str=None, canEmpty:bool=False, insideOf:str=None, uniqueChild:bool=None, definerEncompass:list=None) -> list: 
        ...
        while True:
            ...
            if not next:
                raise HovaSyntaxError(f'Expected "{until}" to close "{encompassName}", but reached EOF.', tokens[pos-1].start_ln, tokens[pos-1].start_col)

            if next.value == until:
                break
            
            if insideOf and context_stack[-1] != insideOf:
                raise HovaContextError(f'This is must be insideOf {insideOf}, so is {context_stack[-1]}', next.start_ln, next.start_col)
            ...
            if uniqueChild:
                for it in nodes:   
                    ...
                    #         return HovaError('202', f'Already exists a {definerEncompass} called "{node['name']}".', next.start_ln)
                        
            if node is None:
                raise HovaSyntaxError(f'Parser stalled in "{encompassName}". Token "{next.value}" not consumed.', next.start_ln, next.start_col)
            
            if pos == before:
                raise HovaSyntaxError(f'Infinite loop detected in "{encompassName}". Token "{next.value}"', next.start_ln, next.start_col)
            
            
            
            if not node['type'] in ('StructLiteral', 'SymbolLiteral'):
                
                if node['type'] in ('AtomicEncompass', 'OreEncompass', 'CaveEncompass'):
                    if encompassName == 'OreEncompass':
        ...
        if IsError(untilTok): raise HovaSyntaxError(f'Expected "end" to close "{encompassName}" keyword', tokens[pos-1].start_ln, tokens[pos-1].start_col)
    ...
    def Parse_Array(until:str=None, arrayName:str=None):
        ...
        while True:
            ...
            if not next:
                raise HovaSyntaxError(f'Expected "{until}" to close "{arrayName}", but reached EOF.', tokens[pos-1].start_ln, tokens[pos-1].start_col)
            ...
            if not ViewNext().value == "]" and ViewNext() and not ViewNext().value == ",":
                raise HovaSyntaxError(f'Expected "," after each item', tokens[pos-1].start_ln, tokens[pos-1].start_col)
            ...
            if node is None:
                raise HovaSyntaxError(f'Parser stalled in "{arrayName}". Token "{next.value}" not consumed', next.start_ln, next.start_col)
                
            if IsError(node): raise node
            
            if pos == before:
                raise HovaSyntaxError(f'Infinite loop detected in "{arrayName}"', next.start_ln, next.start_col)
                
            items.append(node)
            
                
        closedArray = Expect(until)
        if IsError(closedArray): raise HovaSyntaxError(f'Expected "]" to close the Array "{arrayName}"', next.start_ln, next.start_col)
    ...
    # Utils functions to speed-up the development
    ...
    def IsType(value:Token=None, typeList:list=None) -> bool:
        for t in typeList:
            if not t in TypesExtended:
                raise HovaTypeError(f'This {t} type not exist.', value.start_ln, value.start_col)
    ...
    # Parsers for Encompass
    
    def Parse_Anvil():
        if NextIsNone():
            raise HovaSyntaxError("Expected 'AnvilName' after 'anvil' keyword", tokens[pos-1].start_ln, tokens[pos-1].start_col)
        
        tok = tokens[pos]
        start_ln, start_col = tok.start_ln, tok.start_col
        ...
        return AnvilEncompass(
            ...
            start_col=start_col,
            end_ln=tok.start_ln,
            end_col=tok.start_col
        )

    def Parse_Atomic():
        
        if not context_stack or not context_stack[-1] == 'anvil':
            raise HovaContextError('You are declaring an AtomicEncompass outside an AnvilEncompass', tokens[pos-1].start_ln, tokens[pos-1].start_col)
        
        if NextIsNone():
            raise HovaSyntaxError("There is something wrong with AtomicEncompass", tok.start_ln, tok.start_col)
        
        tok = tokens[pos]
        
        if not context_stack[-1] == 'anvil':
            raise HovaContextError("'AtomicEncompass' must be inside an 'AnvilEncompass'.", tok.start_ln, tok.start_col)
        ...
        if not Next():
            raise HovaSyntaxError("Expected 'end' to close 'AtomicEncompass'", tok.start_ln, tok.start_col)
    ...
    def Parse_Cave():
        ...
        if not context_stack or not context_stack[-1] == 'anvil':
            raise HovaContextError('You are declaring an CaveEncompass outside an AnvilEncompass', tokens[pos-1].start_ln, tokens[pos-1].start_col) 
        
        if NextIsNone():
            raise HovaSyntaxError("Expected 'CaveName' after 'cave' keyword", tokens[pos-1].start_ln, tokens[pos-1].start_col)
    ...
    def Parse_Ore():
        ...
        if not context_stack:
            if not context_stack[-1] == 'anvil':
                raise HovaContextError('You are declaring an OreEncompass outside an AnvilEncompass', tokens[pos-1].start_ln, tokens[pos-1].start_col)
            
            if not context_stack[-1] == 'cave':
                raise HovaContextError('You are declaring an OreEncompass outside an AnvilEncompass or CaveEncompass', tokens[pos-1].start_ln, tokens[pos-1].start_col)
        
        if NextIsNone():
            raise HovaSyntaxError("Expected 'OreName' after 'ore' keyword", tok.start_ln, tok.start_col)
            
        tok = tokens[pos]
        
        # If ore is not inside an anvil
        if not context_stack[0] == 'anvil':
            raise HovaContextError("'OreEncompass' must be inside an 'AnvilEncompass'.", tok.start_ln, tok.start_col)
    ...
    def Parse_Spark():
        ...
        if not context_stack or not context_stack[-1] == 'ore':
            raise HovaContextError('You are declaring a SparkDefiner outside an OreEncompass', tok.start_ln, tok.start_col)
        
        if NextIsNone():
            raise HovaSyntaxError('Expected "SparkName" after "spark" literal.', tok.start_ln, tok.start_col)

        # 1 = Sides                           
        SparkNameTok = Expect(type="IDENTIFIER")
        if IsError(SparkNameTok): raise HovaSyntaxError('Expected "SparkName" but something is wrong', tok.start_ln, tok.start_col)
        ...
        if SparkType and not TypeExist(SparkType):
            raise HovaTypeError(f'This type from Spark {SparkName} not exist or is wrong', tok.start_ln, tok.start_col)
        
        # 3 - :
        twoDots = Expect(value=":")
        if IsError(twoDots): raise HovaSyntaxError(f'Expected spark finished. You can put ":" or a type value (optional).', tok.start_ln, tok.start_col)
        ...
        if SparkType is not None and not TypeIsCorrect(SparkValue, SparkType):
            raise HovaTypeError(f'The Spark "{SparkName}" has a value with different type of {SparkType}', tok.start_ln, tok.start_col)
    ...
    def Parse_Atom():
        ...
        if not context_stack or not context_stack[-1] == 'atomic':
            raise HovaContextError('You are declaring an AtomDefiner outside an AtomicEncompass', tok.start_ln, tok.start_col)
        
        if NextIsNone():
            raise HovaSyntaxError('Expected "AtomName" after "atom" literal.', tok.start_ln, tok.start_col)
        ...
        if IsError(AtomNameTok): raise HovaSyntaxError('Expected "AtomName" after "atom" literal.', tok.start_ln, tok.start_col)
    
        twoDots = Expect(value=":")
        if IsError(twoDots): raise HovaSyntaxError(f'Expected ":" after "{AtomName}"', tok.start_ln, tok.start_col)
        
        if not DefinerValueIsCorrect('StringLiteral') or ViewNext().value == "end":
            raise HovaTypeError(f'Expected a "str" to AtomDefiner', tok.start_ln, tok.start_col)
        ...
        if not isTyped:
            raise HovaTypeError(f'Expected a "StringValue" to AtomDefiner', tok.start_ln, tok.start_col)
    ...
    def Parse_Sufx():
        ...
        valueTok = Expect(type="STRING")
        if IsError(valueTok): raise valueTok
    ...
    def Parse_Dimension():
        ...
        pathTok = Expect(type="IDENTIFIER")
        if IsError(pathTok): raise pathTok
    ...
    def Parse_Primary():
        ...
        if token.type == "KEYWORD": 
            ...
            if keyword == "anvil": return Parse_Anvil()
            if keyword == 'ore': return Parse_Ore()  
            if keyword == "atomic": return Parse_Atomic()
            if keyword == 'cave': return Parse_Cave()  
            
            # Definers Conditions
            
            if keyword == 'spark': return Parse_Spark()   
            if keyword == 'atom': return Parse_Atom()   

            # Others
            
            if keyword == 'dimension': return Parse_Dimension()
            
            # OfStructure
            
            if keyword in ("true", "false"): return Literal("BooleanLiteral", token.value)  

        if token.type == "IDENTIFIER": return Identifier(token.value)
        
        raise HovaSyntaxError(f'Token "{token.value}" not exist', token.start_ln, token.start_col)
    ...
    while ViewNext():
        ...
        if IsError(ast_node): return ast_node   
