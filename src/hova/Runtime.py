...
# NOTE: Hova 0.1.9 -> 2026 January, 15 "O Hova não é grande porque tem muitas features. Ele é grande porque não tenta ser o que não é."
# [DONE] 
# XXX - Set "atomic" as optional and not mandatory to Emite
# XXX - Set a default atomic to Emit works, as emit="json" and outVisual="default"
# XXX - Improves AtomBehaviours(AtomsHova) names, less engine-config-like or namespaces like object.property
# XXX - Separate "hova.property" to custom atoms.
# XXX - Create "hova.clean" to minize info from values
# XXX - Removed Rune feature
# XXX - Remove 'be' as block opener from all encompasses
# XXX - Add new encompass 'cave' like Container for Ores
# XXX - Let spark type as OPTIONAL (int or nothing)
# XXX - Change Output build Hova folder name to 'hovabuild' and let type's folder name lowercase style
# XXX - Remove Detail and Temper features
# XXX - TEST HOVA CODE
...
def Forge(input, output_dir=None, force_emit=None):
    ...
    if justKeyword.count("charm") > 1:
        raise HovaEmissionError("Hova file must have only one 'CharmEncompass'", 0, 0)
